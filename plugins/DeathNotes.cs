...
{
    [Info("Death Notes", "LaserHydra", "5.2.20", ResourceId = 819)]
    ...
    class DeathNotes : RustPlugin
    {
        ...
        class Attacker
        {
            ...
            public string TryGetName()
            {
                ...
                if (type == AttackerType.Murderer)
                    return entity?.ToPlayer()?.displayName ?? "Murderer";
	            if (type == AttackerType.Scientist)
		            return entity?.ToPlayer()?.displayName ?? "Scientist";
        ...
        class DeathData
        {
            ...
            public DeathReason TryGetReason()
            {
                ...
                if (attacker.type == AttackerType.Murderer)
                    return DeathReason.Murderer;
                if (victim.type == VictimType.Murderer)
                    return DeathReason.MurdererDeath;
        ...
        void LoadConfig()
        {
            ...
            SetConfig("Messages", "Murderer", new List<object> { "A {attacker} haunted down {victim}." });
            SetConfig("Messages", "MurdererDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." });
	        SetConfig("Messages", "Scientist", new List<object> { "A {attacker} got rid of {victim}." });
	        SetConfig("Messages", "ScientistDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." });
            ...
            Messages = GetConfig(new Dictionary<string, object>
            {
                ...
                { "Murderer", new List<object> { "A {attacker} haunted down {victim}." }},
                { "MurdererDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." }},
	            { "Scientist", new List<object> { "A {attacker} got rid of {victim}." }},
	            { "ScientistDeath", new List<object> { "{attacker} killed a {victim} with a {weapon}{attachments} from {distance}m." }},
